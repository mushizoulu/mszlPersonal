const { locales, itemLimit } = window.siteData;
...
function renderWorks(localeData) {
  ...
  limitItems(localeData.works).forEach((work) => {
    ...
    article.innerHTML = `
      ...
      <a class="card-link" href="${work.link}" target="_blank" rel="noreferrer">${work.linkLabel}</a>
    `;
...
function applyLocale(localeKey) {
  ...
  document.getElementById("spotlight-link").setAttribute("href", "#works");
