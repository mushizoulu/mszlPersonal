const worksDataReady = window.siteDataReady || Promise.resolve(window.siteData);

worksDataReady.then((siteData) => {
  const worksLocales = siteData.locales;
  const worksPerPage = siteData.itemLimit;
  ...
  let worksLocale = worksLocales[window.localStorage.getItem("site-locale")]
