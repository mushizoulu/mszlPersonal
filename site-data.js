window.siteData = {
  itemLimit: 10,
  locales: {
    "zh-Hans": {
      site: {
        htmlLang: "zh-Hans",
        title: "むしぞる中尉 | 翻译作品、推荐清单与博客",
        description: "展示翻译作品、推荐清单与博客文章的个人网站。",
        name: "むしぞる中尉",
        tagline: "翻译、推荐与写作",
        heroNote: "Bilingual craft for readers with taste",
        heroTitle: "如果日本右翼把外国人都赶出日本，那3K工作谁来做？",
        heroDescription: "我的翻译作品、我愿意公开推荐的商品，以及我写的各种稀奇古怪的博客文章。",
        latestLabel: "最新发布",
        latestTitle: "翻译与博客",
        latestWorksHeading: "最新翻译作品（点击可进入商品页面）",
        latestPostsHeading: "最新博客文章（点击可进入阅读页面）",
        about: "社团名：むしぞる中尉。\n出身地：中共国。\n爱好：ACGN，网球，F1。\n最喜欢的声优：楠木ともり。\n\n关于本站\n\n本站是由个人独立运营和维护的内容网站，主要发布翻译、语言表达、作品介绍、个人博客及商品推广相关内容。\n本站关注日本动画、漫画、游戏、音声作品、轻小说及其他文化娱乐内容，也会分享翻译过程中遇到的用词选择、表达差异、作品理解、文化背景和个人感想。\n除翻译与评论内容外，本站也可能发布部分商品介绍、作品推荐、购买参考或推广信息。相关内容会尽量基于实际资料、个人理解和使用体验进行整理，供读者参考。\n本站内容由站长自行编辑和维护。除特别注明外，文章观点仅代表作者个人看法，不代表任何平台、厂商、社团或作品版权方的立场。\n本站希望以长期积累原创内容为目标，为关注翻译、日本文化娱乐内容以及相关商品信息的读者提供有参考价值的文章。",
        contactTitle: "如果你想合作、约稿或交换想法",
        contactCopy: "欢迎通过邮箱联系我，我会尽快回复。",
        contactEmail: "mushizoulu@yahoo.co.jp",
        worksMoreText: "查看所有翻译作品",
        blogMoreText: "查看所有博客",
        affiliateDisclosure: "",
        footer: "© 2026 むしぞる中尉. All rights reserved."
      },
      nav: {
        works: "翻译作品",
        blog: "博客",
        picks: "推荐清单",
        about: "关于我",
        contact: "合作联系"
      },
      switcher: {
        hans: "简体",
        hant: "繁體",
        ja: "日本語"
      },
      buttons: {
        primary: "先看推荐",
        secondary: "进入博客",
        viewProduct: "查看推荐产品页面",
        readArticle: "阅读全文",
        backHome: "返回首页",
        backBlog: "返回博客",
        backPicks: "返回推荐清单",
        newer: "上一页",
        older: "下一页",
        menu: "菜单"
      },
      sections: {
        worksEyebrow: "Featured Translations",
        worksTitle: "翻译作品",
        worksDescription: "展示最近上架的九部翻译作品。点击标题可以直接跳转到对应的 DLsite 商品页面。",
        blogEyebrow: "Notes & Essays",
        blogTitle: "博客",
        blogDescription: "翻译随想、阅读、奇怪的小说创作、突发奇想……",
        picksEyebrow: "Curated Affiliate Picks",
        picksTitle: "推荐清单",
        picksDescription: "我喜欢的作品、商品",
        aboutEyebrow: "About",
        aboutTitle: "关于我",
        contactEyebrow: "Contact"
      },
      stats: [
        { value: "0", label: "翻译作品总数" },
        { value: "0", label: "博客总数" },
        { value: "0", label: "建站天数" }
      ],
      worksPage: {
        title: "翻译作品",
        intro: "这里集中展示翻译作品。每页最多显示十部作品，点击标题会直接跳到对应的 DLsite 商品页面。",
        pageLabel: "第 {page} 页 / 共 {total} 页"
      },
      blogPage: {
        title: "博客",
        intro: "这里列出全部博客文章。每页最多显示十篇，点击标题可以阅读完整正文。",
        pageLabel: "第 {page} 页 / 共 {total} 页"
      },
      picksPage: {
        title: "推荐清单",
        intro: "这里显示所有推荐商品。每条内容展示标题和推荐文章开头部分，点击标题可阅读完整推荐文章。",
        pageLabel: "第 {page} 页 / 共 {total} 页",
        viewProductText: "查看推荐产品页面"
      },
      postPage: {
        missingTitle: "未找到这篇文章",
        missingBody: "你要查看的博客文章不存在，或者已经被移除。",
        previous: "上一篇",
        next: "下一篇"
      },
      pickPostPage: {
        missingTitle: "未找到这篇推荐文章",
        missingBody: "你要查看的推荐文章不存在，或者已经被移除。"
      },
      share: {
        title: "分享到",
        x: "X",
        threads: "Threads",
        weibo: "微博",
        copyLink: "复制链接",
        copySuccess: "链接已复制",
        threadsCopied: "已复制标题、摘要和链接",
        copyFailed: "复制失败，请手动复制"
      },
      works: [],
      posts: [],
      picks: []
    },
    "zh-Hant": {
      site: {
        htmlLang: "zh-Hant",
        title: "むしぞる中尉 | 翻譯作品、推薦清單與部落格",
        description: "展示翻譯作品、推薦清單與部落格文章的個人網站。",
        name: "むしぞる中尉",
        tagline: "翻譯、推薦與寫作",
        heroNote: "Bilingual craft for readers with taste",
        heroTitle: "如果日本右翼把外國人都趕出日本，那3K工作誰來做？",
        heroDescription: "我的翻譯作品、我願意公開推薦的商品，以及我寫的各種稀奇古怪的博客文章。",
        latestLabel: "最新發布",
        latestTitle: "翻譯與部落格",
        latestWorksHeading: "最新翻譯作品（點擊可進入商品頁面）",
        latestPostsHeading: "最新部落格文章（點擊可進入閱讀頁面）",
        about: "社團名：むしぞる中尉。\n出身地：中共國。\n愛好：ACGN，網球，F1。\n最喜歡的聲優：楠木ともり。\n\n關於本站\n\n本站是由個人獨立運營和維護的內容網站，主要發佈翻譯、語言表達、作品介紹、個人博客及商品推廣相關內容。\n本站關注日本動畫、漫畫、遊戲、音聲作品、輕小說及其他文化娛樂內容，也會分享翻譯過程中遇到的用詞選擇、表達差異、作品理解、文化背景和個人感想。\n除翻譯與評論內容外，本站也可能發佈部分商品介紹、作品推薦、購買參考或推廣信息。相關內容會盡量基於實際資料、個人理解和使用體驗進行整理，供讀者參考。\n本站內容由站長自行編輯和維護。除特別註明外，文章觀點僅代表作者個人看法，不代表任何平臺、廠商、社團或作品版權方的立場。\n本站希望以長期積累原創內容爲目標，爲關注翻譯、日本文化娛樂內容以及相關商品信息的讀者提供有參考價值的文章。",
        contactTitle: "如果你想合作、約稿或交換想法",
        contactCopy: "歡迎透過電子郵件聯絡我，我會在合適的時候回覆。",
        contactEmail: "mushizoulu@yahoo.co.jp",
        worksMoreText: "查看所有翻譯作品",
        blogMoreText: "查看所有部落格",
        affiliateDisclosure: "",
        footer: "© 2026 むしぞる中尉. All rights reserved."
      },
      nav: {
        works: "翻譯作品",
        blog: "部落格",
        picks: "推薦清單",
        about: "關於我",
        contact: "合作聯絡"
      },
      switcher: {
        hans: "简体",
        hant: "繁體",
        ja: "日本語"
      },
      buttons: {
        primary: "先看推薦",
        secondary: "進入部落格",
        viewProduct: "查看推薦產品頁面",
        readArticle: "閱讀全文",
        backHome: "返回首頁",
        backBlog: "返回部落格",
        backPicks: "返回推薦清單",
        newer: "上一頁",
        older: "下一頁",
        menu: "選單"
      },
      sections: {
        worksEyebrow: "Featured Translations",
        worksTitle: "翻譯作品",
        worksDescription: "展示最近上架的九部翻譯作品。點擊標題可以直接跳轉到對應的 DLsite 商品頁面。",
        blogEyebrow: "Notes & Essays",
        blogTitle: "部落格",
        blogDescription: "翻譯隨想、閱讀、奇怪的小說創作、突發奇想……",
        picksEyebrow: "Curated Affiliate Picks",
        picksTitle: "推薦清單",
        picksDescription: "我喜歡的作品、商品",
        aboutEyebrow: "About",
        aboutTitle: "關於我",
        contactEyebrow: "Contact"
      },
      stats: [
        { value: "0", label: "翻譯作品總數" },
        { value: "0", label: "部落格總數" },
        { value: "0", label: "建站天數" }
      ],
      worksPage: {
        title: "翻譯作品",
        intro: "這裡集中展示翻譯作品。每頁最多顯示十部作品，點擊標題會直接跳到對應的 DLsite 商品頁面。",
        pageLabel: "第 {page} 頁 / 共 {total} 頁"
      },
      blogPage: {
        title: "部落格",
        intro: "這裡列出全部部落格文章。每頁最多顯示十篇，點擊標題可以閱讀完整正文。",
        pageLabel: "第 {page} 頁 / 共 {total} 頁"
      },
      picksPage: {
        title: "推薦清單",
        intro: "這裡顯示所有推薦商品。每條內容展示標題和推薦文章開頭部分，點擊標題可閱讀完整推薦文章。",
        pageLabel: "第 {page} 頁 / 共 {total} 頁",
        viewProductText: "查看推薦產品頁面"
      },
      postPage: {
        missingTitle: "找不到這篇文章",
        missingBody: "你要查看的部落格文章不存在，或者已經被移除。",
        previous: "上一篇",
        next: "下一篇"
      },
      pickPostPage: {
        missingTitle: "找不到這篇推薦文章",
        missingBody: "你要查看的推薦文章不存在，或者已經被移除。"
      },
      share: {
        title: "分享到",
        x: "X",
        threads: "Threads",
        weibo: "微博",
        copyLink: "複製連結",
        copySuccess: "連結已複製",
        threadsCopied: "已複製標題、摘要和連結",
        copyFailed: "複製失敗，請手動複製"
      },
      works: [],
      posts: [],
      picks: []
    },
    "ja": {
      site: {
        htmlLang: "ja",
        title: "むしぞる中尉 | 翻訳作品・おすすめ・ブログ",
        description: "翻訳作品、おすすめ記事、ブログをまとめた個人サイトです。",
        name: "むしぞる中尉",
        tagline: "翻訳・おすすめ・ブログ",
        heroNote: "Bilingual craft for readers with taste",
        heroTitle: "もし日本の右翼が外国人を全員日本から追い出したら、3Kの仕事は誰がやるのか？",
        heroDescription: "私が翻訳した作品や、自信を持っておすすめの商品、そして私が書いた様々なユニークで怪奇なブログ記事を公開しています。",
        latestLabel: "最近発表",
        latestTitle: "翻訳とブログ",
        latestWorksHeading: "最新の翻訳作品（クリックで商品ページに移動）",
        latestPostsHeading: "最新のブログ記事（クリックで閲覧ページに移動）",
        about: "サークル名：むしぞる中尉。\n出身地：中国。\n趣味：ACGN、テニス、F1。\n一番好きな声優：楠木ともり。\n\n当サイトについて\n\n当サイトは、個人が独自に運営・管理しているコンテンツサイトであり、主に翻訳、言語表現、作品紹介、個人ブログ、および商品プロモーションに関するコンテンツを公開しています。\n当サイトは、日本のアニメ、漫画、ゲーム、音声作品、ライトノベル、その他の文化・エンターテインメントコンテンツに焦点を当てており、翻訳過程で遭遇した用語の選択、表現の違い、作品への理解、文化的背景、および個人的な感想についても共有しています。\n翻訳や評論の内容に加え、当サイトでは一部の商品紹介、作品のおすすめ、購入の参考情報、またはプロモーション情報を掲載する場合があります。関連する内容は、実際の資料、個人的な理解、および使用体験に基づいて整理し、読者の参考となるよう努めます。\n当サイトのコンテンツはサイト管理者が独自に編集・管理しています。特に明記されていない限り、記事の見解は著者の個人的な見解であり、いかなるプラットフォーム、メーカー、団体、または作品の著作権者の立場を代表するものではありません。\n当サイトは、長期的にオリジナルコンテンツを蓄積することを目標とし、翻訳、日本の文化・エンターテインメントコンテンツ、および関連商品情報に関心を持つ読者の皆様に、参考となる記事を提供していきたいと考えています。",
        contactTitle: "お仕事・依頼・感想交換はこちら",
        contactCopy: "メールでご連絡ください。なるべく早く返信します。",
        contactEmail: "mushizoulu@yahoo.co.jp",
        worksMoreText: "すべての翻訳作品を見る",
        blogMoreText: "すべてのブログを見る",
        affiliateDisclosure: "",
        footer: "© 2026 むしぞる中尉. All rights reserved."
      },
      nav: {
        works: "翻訳作品",
        blog: "ブログ",
        picks: "おすすめ",
        about: "プロフィール",
        contact: "連絡先"
      },
      switcher: {
        hans: "简体",
        hant: "繁體",
        ja: "日本語"
      },
      buttons: {
        primary: "おすすめを見る",
        secondary: "ブログへ",
        viewProduct: "商品ページを見る",
        readArticle: "全文を読む",
        backHome: "トップへ戻る",
        backBlog: "ブログへ戻る",
        backPicks: "おすすめ一覧へ戻る",
        newer: "前のページ",
        older: "次のページ",
        menu: "メニュー"
      },
      sections: {
        worksEyebrow: "Featured Translations",
        worksTitle: "翻訳作品",
        worksDescription: "最近追加された翻訳作品9点を紹介します。タイトルをクリックすると、対応するDLsiteの商品ページに直接移動します。",
        blogEyebrow: "Notes & Essays",
        blogTitle: "ブログ",
        blogDescription: "翻訳の雑感、読書、奇妙な創作小説、突発的な思いつき……",
        picksEyebrow: "Curated Affiliate Picks",
        picksTitle: "おすすめ",
        picksDescription: "私が好きな作品や商品",
        aboutEyebrow: "About",
        aboutTitle: "プロフィール",
        contactEyebrow: "Contact"
      },
      stats: [
        { value: "0", label: "翻訳作品総数" },
        { value: "0", label: "ブログ総数" },
        { value: "0", label: "開設日数" }
      ],
      worksPage: {
        title: "翻訳作品",
        intro: "翻訳作品をまとめて表示します。1ページに最大10件まで表示され、タイトルをクリックすると該当するDLsite商品ページへ移動します。",
        pageLabel: "{page} / {total} ページ"
      },
      blogPage: {
        title: "ブログ",
        intro: "すべてのブログ記事を表示します。1ページに最大10件まで表示され、タイトルをクリックすると全文ページへ移動します。",
        pageLabel: "{page} / {total} ページ"
      },
      picksPage: {
        title: "おすすめ",
        intro: "おすすめしたい商品を一覧表示します。各項目にはタイトルと記事冒頭を表示し、タイトルをクリックすると全文ページへ移動できます。",
        pageLabel: "{page} / {total} ページ",
        viewProductText: "商品ページを見る"
      },
      postPage: {
        missingTitle: "記事が見つかりません",
        missingBody: "指定されたブログ記事は存在しないか、すでに削除されています。",
        previous: "前の記事",
        next: "次の記事"
      },
      pickPostPage: {
        missingTitle: "おすすめ記事が見つかりません",
        missingBody: "指定されたおすすめ記事は存在しないか、すでに削除されています。"
      },
      share: {
        title: "Share",
        x: "X",
        threads: "Threads",
        weibo: "Weibo",
        copyLink: "リンクをコピー",
        copySuccess: "リンクをコピーしました",
        threadsCopied: "タイトル、抜粋、リンクをコピーしました",
        copyFailed: "コピーできませんでした。手動でコピーしてください"
      },
      works: [],
      posts: [],
      picks: []
    }
  }
};
